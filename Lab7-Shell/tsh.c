...
int builtin_cmd(char **argv) 
{
	...
	if(!strcmp(argv[0],"quit"))									//判断是否为quit命令
        exit(0);												//结束程序
	else if(!strcmp(argv[0], "fg") || !strcmp(argv[0], "bg")){	//判断是否为fg或bg命令
		do_bgfg(argv);
		return 1;
	}
	else if(!strcmp(argv[0], "jobs")){							//判断是否为jobs命令
		sigprocmask(SIG_BLOCK, &mask, &prev);					//访问全局变量，阻塞所有信号
		listjobs(jobs);
...
void do_bgfg(char **argv) 
{
    ...
    if (argv[1] == NULL) {		//命令无参数，忽略并输出提示
	printf("%s command requires PID or %%jobid argument\n", argv[0]);
	return;
    }
    
    /* Parse the required PID or %JID arg */
    if (isdigit(argv[1][0])) {		//判断参数是否只有整数
    ...
    else if (argv[1][0] == '%') {	//判断参数是否为%加数字的形式
...
{
    sigset_t mask, prev;
    if(sigemptyset(&mask) < 0)						//初始化mask为空
    	unix_error("sigemptyset error");
    if(sigaddset(&mask, SIGCHLD) < 0)
		unix_error("sigaddset error");
    if(sigprocmask(SIG_BLOCK, &mask, &prev) < 0)	//阻塞信号
		unix_error("sigprocmask error");
    while(pid == fgpid(jobs))
        if(sigsuspend(&prev) != -1)					//挂起
...
void sigchld_handler(int sig) 
{
	...
	while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0){
		if(sigprocmask(SIG_BLOCK, &mask, &prev) < 0)	//阻塞
			unix_error("sigpromask error"); 
		if(WIFEXITED(status)) 			//进程正常终止返回真，从列表中删除job
		    deletejob(jobs, pid);
		else if(WIFSIGNALED(status)){	//进程因为收到信号而终止返回真，从列表中删除job
			printf("Job [%d] (%d) terminated by signal %d\n", pid2jid(pid), pid, WTERMSIG(status));
		    deletejob(jobs, pid);
		}
	    else if(WIFSTOPPED(status)){	//进程被挂起
		    cur_job = getjobpid(jobs, pid);
		    cur_job->state = ST;				//不删除job，而是设为挂起状态
